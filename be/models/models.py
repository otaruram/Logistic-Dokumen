...
class Scan(Base):
    ...
    is_fraud_scan = Column(Boolean, default=False, nullable=False)  # True = fraud detection mode
    
    # Fraud-specific fields (only populated when is_fraud_scan=True)
    fraud_nominal_total = Column(Float, nullable=True)
    fraud_nama_klien = Column(String(255), nullable=True)
    fraud_nomor_surat_jalan = Column(String(255), nullable=True)
    fraud_tanggal_jatuh_tempo = Column(String(100), nullable=True)
    fraud_confidence = Column(String(20), nullable=True)  # low, medium, high
