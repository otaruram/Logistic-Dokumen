...
import { Search, Trash2, CloudUpload, Pencil, Save, Clock, ImageIcon, FileText, X } from "lucide-react";
...
const DataTable = ({ logs, onDeleteLog, onUpdateLog }: DataTableProps) => {
  ...
  const filteredLogs = useMemo(() => {
    return logs.filter((log) => {
      ...
      // Konversi index bulan (0-11) ke string ("01"-"12")
  ...
  const handleDriveUpload = async () => {
    ...
    try {
      ...
      } else { toast.error("Gagal Upload ke Drive"); }
    ...
    <div className="w-full space-y-4">
      ...
      <div className="bg-white dark:bg-zinc-900 p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-zinc-800">
        ...
        <div className="flex flex-col md:flex-row justify-between gap-4 mb-4 items-center">
            <div className="relative w-full md:w-1/2">
                ...
                <input 
                    ...
                    className="w-full pl-9 pr-4 py-2 bg-gray-50 dark:bg-zinc-800 border-none rounded-xl text-sm focus:ring-2 focus:ring-black/5 outline-none transition-all" 
                />
            </div>
            
            {/* 🔥 TOMBOL EXPORT (DIKEMBALIKAN) 🔥 */}
            <Button onClick={handleDriveUpload} disabled={isUploading} className="bg-green-600 hover:bg-green-700 text-white rounded-xl h-10 text-xs font-bold w-full md:w-auto shadow-sm shadow-green-200">
                {isUploading ? <span className="animate-pulse">Mengupload...</span> : <><CloudUpload className="w-4 h-4 mr-2" /> EXPORT DRIVE</>}
            </Button>
        </div>
        ...
        <div className="flex flex-wrap gap-2 items-center">
            {/* TGL */}
            <select value={filterDay} onChange={(e) => setFilterDay(e.target.value)} className="p-2 rounded-lg bg-gray-50 dark:bg-zinc-800 text-xs border-none cursor-pointer outline-none">
                <option value="ALL">Tgl</option>{days.map(d => <option key={d} value={d}>{d}</option>)}
            </select>

            {/* BLN */}
            <select value={filterMonth} onChange={(e) => setFilterMonth(e.target.value)} className="p-2 rounded-lg bg-gray-50 dark:bg-zinc-800 text-xs border-none cursor-pointer outline-none">
                <option value="ALL">Bln</option>{months.map((m, i) => <option key={m} value={i}>{m}</option>)}
            </select>

            {/* THN */}
            <select value={filterYear} onChange={(e) => setFilterYear(e.target.value)} className="p-2 rounded-lg bg-gray-50 dark:bg-zinc-800 text-xs border-none cursor-pointer outline-none">
                <option value="ALL">Thn</option>{years.map(y => <option key={y} value={y}>{y}</option>)}
            </select>

            {/* JAM */}
            <Popover>
                <PopoverTrigger asChild>
                    <Button variant="outline" className="border-none bg-gray-50 dark:bg-zinc-800 text-gray-600 dark:text-gray-300 h-9 text-xs">
                        <Clock className="w-3 h-3 mr-1" />
                        {filterHour !== "ALL" ? `${filterHour}:${filterMinute !== "ALL" ? filterMinute : '00'}` : "Jam"}
                    </Button>
                </PopoverTrigger>
                <PopoverContent className="w-56 p-3 rounded-xl border-gray-100 dark:border-zinc-700">
                    <div className="flex gap-2">
                        <select value={filterHour} onChange={(e) => setFilterHour(e.target.value)} className="w-full p-2 bg-gray-50 rounded text-xs"><option value="ALL">Jam</option>{hours.map(h => <option key={h} value={h}>{h}</option>)}</select>
                        <select value={filterMinute} onChange={(e) => setFilterMinute(e.target.value)} className="w-full p-2 bg-gray-50 rounded text-xs"><option value="ALL">Menit</option>{minutes.map(m => <option key={m} value={m}>{m}</option>)}</select>
                    </div>
                </PopoverContent>
            </Popover>
            ...
            {(filterDay !== "ALL" || filterMonth !== "ALL" || filterYear !== "ALL" || filterHour !== "ALL") && (
                <Button variant="ghost" onClick={() => { setFilterDay("ALL"); setFilterMonth("ALL"); setFilterYear("ALL"); setFilterHour("ALL"); setFilterMinute("ALL"); }} className="text-red-500 hover:text-red-600 hover:bg-red-50 h-9 text-xs">
                    Reset
                </Button>
            )}
        </div>
                ...
                filteredLogs.map((log, index) => (
                    ...
                    <td className="px-6 py-4">
                        <div className="flex flex-col">
                            <span className="text-xs font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-md w-fit mb-1 border border-blue-100">{log.docType}</span>
                    ...
                    <td className="px-6 py-4 max-w-xs whitespace-normal">
                        {editingLogId === log.id ? (
                        <div className="flex gap-2">
                            <textarea value={editingSummary} onChange={(e) => setEditingSummary(e.target.value)} className="w-full border rounded-lg px-2 py-1 text-sm bg-white focus:ring-2 focus:ring-blue-500 outline-none" rows={2} autoFocus />
      ...
      {/* --- IMAGE MODAL (ZOOM) --- */}
      {zoomedImage && (
        <div className="fixed inset-0 bg-black/90 z-[9999] flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in" onClick={() => setZoomedImage(null)}>
          <img src={zoomedImage} className="max-w-full max-h-[85vh] rounded-xl shadow-2xl animate-in zoom-in-95" />
          <button className="absolute top-4 right-4 text-white/70 hover:text-white bg-black/50 p-2 rounded-full"><X className="w-6 h-6" /></button>
        </div>
      )}
    </div>
  );
