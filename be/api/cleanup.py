...
from services.credit_service import MAX_CREDITS

router = APIRouter()

# Secret key for cleanup endpoint (set CLEANUP_SECRET in .env)
CLEANUP_SECRET = os.getenv("CLEANUP_SECRET", "")
ENABLE_MONTHLY_CLEANUP = os.getenv("ENABLE_MONTHLY_CLEANUP", "false").lower() == "true"
...
async def trigger_monthly_cleanup(authorization: Optional[str] = Header(None)):
    ...
    """
    try:
        if not ENABLE_MONTHLY_CLEANUP:
            raise HTTPException(
                status_code=410,
                detail="Monthly cleanup is disabled (ENABLE_MONTHLY_CLEANUP=false).",
            )

        # Verify authorization
        if not CLEANUP_SECRET:
            raise HTTPException(status_code=503, detail="CLEANUP_SECRET is not configured")
...
async def trigger_daily_credit_reset(authorization: Optional[str] = Header(None)):
    """
    Daily credit top-up: +1 credit per user/day (max 10)
    Requires authorization header with secret key
    Admin user is exempt from automated updates
    """
    try:
        # Verify authorization
        if not CLEANUP_SECRET:
        ...
        # Fetch candidates below max credits
        q = supabase_admin.table("profiles").select("id,credits").lt("credits", MAX_CREDITS)
        if admin_id:
            q = q.neq("id", admin_id)
        ...
        for row in rows:
            uid = row.get("id")
            cur = int(row.get("credits", 0) or 0)
            nxt = min(MAX_CREDITS, cur + 1)
            if nxt != cur:
                supabase_admin.table("profiles").update({"credits": nxt}).eq("id", uid).execute()
                updated_count += 1
        
        response = {
            "timestamp": datetime.now().isoformat(),
            "users_updated": updated_count,
            "credit_cap": MAX_CREDITS,
            "bonus_per_day": 1,
            "admin_excluded": admin_email,
            "message": "Daily credit top-up completed — +1 credit/user up to max 10 (admin excluded)"
        }
        
        print(f"✅ Daily credit top-up: {updated_count} users received +1 credit (capped at {MAX_CREDITS})")
        return response
