...
async def update_user_credits(
    ...
    try:
        # Fetch current email for upsert if it doesn't exist
        user_email = ""
        local_user = db.query(User).filter(User.id == user_id).first()
        if local_user:
            user_email = local_user.email

        sb.table("profiles").upsert({"id": user_id, "user_email": user_email, "credits": body.credits}, on_conflict="id").execute()
        
        # Update local DB to stay in sync
        if local_user:
            local_user.credits = body.credits
            db.commit()
