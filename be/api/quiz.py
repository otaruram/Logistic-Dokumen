...
PENTING: Buat TEPAT 20 pertanyaan seperti contoh di atas."""
        ...
        if "questions" not in quiz_data or len(quiz_data["questions"]) == 0:
            raise ValueError(f"Invalid quiz structure: No questions found.")
        
        # Soft validation: Just log warning if mismatch, don't fail
        generated_count = len(quiz_data["questions"])
        if generated_count != num_questions:
            print(f"⚠️ Warning: Requested {num_questions} questions, but AI generated {generated_count}. Proceeding anyway.")
            # Update the number of questions to what was actually generated to avoid index errors later
            num_questions = generated_count
