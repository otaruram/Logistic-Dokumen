...
import { Search, Trash2, CloudUpload, ExternalLink, Pencil, Save, X } from "lucide-react";
  ...
  const handleDriveUpload = async () => {
    ...
    try {
      ...
      if (res.status === 'success') {
        toast({ title: "Upload Berhasil", description: "Cek Google Drive Anda" });
        window.open(res.drive_url, '_blank');
      } else {
        toast({ title: "Gagal Upload", description: res.message, variant: "destructive" });
      }
    } catch (e) {
    ...
    <div className="brutal-border overflow-hidden bg-white mt-6">
      <div className="bg-black text-white px-4 py-3 flex justify-between items-center">
        ...
        <Button 
          ...
          className="text-black bg-white border-white hover:bg-gray-200 h-8 text-xs font-bold"
        >
      ...
      <div className="p-4 border-b-2 border-black bg-gray-50">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Cari dokumen, penerima..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full pl-10 pr-4 py-2 border-2 border-black focus:outline-none focus:ring-0 text-sm font-mono"
          />
        </div>
      </div>
                ...
                <td className="px-4 py-3 border-r border-black">
                  {log.imageUrl ? (
                    <img 
                      ...
                      className="w-16 h-10 object-cover border border-black cursor-pointer hover:opacity-80"
                      onError={(e) => {
                  ...
                  {editingLogId === log.id ? (
                    <div className="flex gap-2">
                      <input 
                        ...
                        autoFocus
                      />
                      <button onClick={handleSaveEdit} className="bg-green-500 text-white p-1 border border-black">
                    ...
                    <div>
                      ...
                      <div className="text-[10px] bg-gray-200 inline-block px-1 mt-1 border border-black">
                        {log.receiver}
                      </div>
                    </div>
                  )}
                ...
                <td className="px-4 py-3 text-center">
                  <div className="flex justify-center gap-2">
                    <Button
                      ...
                      className="h-8 w-8 p-0 border border-black hover:bg-yellow-200"
                    >
                    ...
                    <Button
                      ...
                      className="h-8 w-8 p-0 border border-black hover:bg-red-200 text-red-600"
                    >
      ...
      {zoomedImage && (
        ...
        >
          <div className="relative">
            <button onClick={() => setZoomedImage(null)} className="absolute -top-10 right-0 text-white font-bold">
              <X className="w-8 h-8" />
            </button>
