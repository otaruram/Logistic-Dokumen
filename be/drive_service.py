...
def upload_image_to_drive(access_token, image_path, folder_name=None):
    """Upload image file to Google Drive and return public link"""
    try:
        service = get_drive_service_with_token(access_token)
        ...
        # Upload file
        result = upload_file_to_drive(service, file_content, file_name, mime_type, folder_id, False)
        ...
        # Get direct link for image viewing
        direct_link = f"https://drive.google.com/uc?export=view&id={file_id}"
        result['direct_link'] = direct_link
        
        print(f"✓ Image uploaded and made public: {file_name}")
        return result
